...
function Get-VendorCodeWorkbookPath {
    ...
    $toolPreferred = Join-Path $WorkspaceRoot "reference_data\廠商代號.xlsx"
    if (Test-Path -LiteralPath $toolPreferred) {
        return (Resolve-Path -LiteralPath $toolPreferred).Path
    }
...
if ([string]::IsNullOrWhiteSpace($NewProductTemplate)) {
    $toolTemplate = Join-Path $workspace "reference_data\建檔用.xls"
    if (Test-Path -LiteralPath $toolTemplate) {
        $NewProductTemplate = $toolTemplate
    } else {
        $NewProductTemplate = Join-Path $workspace "參考資料\建檔用.xls"
    }
}
if ([string]::IsNullOrWhiteSpace($PurchaseTemplate)) {
    $toolTemplate = Join-Path $workspace "reference_data\採購單匯入範例.xls"
    if (Test-Path -LiteralPath $toolTemplate) {
        $PurchaseTemplate = $toolTemplate
    } else {
