...
進貨圖片擷取預設先使用本機 PaddleOCR，不先使用 AI 視覺 OCR。
照片預設只產生一張 OCR 暫存圖：必要時旋轉後做灰階、對比與輕微銳化，再只用這張圖片跑 PaddleOCR。若 `rotation auto` 第一次結果幾乎沒有欄位關鍵字或貨號命中，才自動改試相反旋轉方向一次；最佳版本以可解析商品列、金額核對通過列數、欄位關鍵字與貨號命中綜合評分。
