...
照片預設只產生一張 OCR 暫存圖：必要時旋轉後用 OpenCV 做灰階、去雜訊、局部對比與輕微銳化，再只用這張圖片跑 PaddleOCR。若 `rotation auto` 第一次結果幾乎沒有欄位關鍵字或貨號命中，才自動改試相反旋轉方向一次；最佳版本以可解析商品列、金額核對通過列數、欄位關鍵字與貨號命中綜合評分。OpenCV 不可用時才退回 PIL 灰階/對比/銳化。
