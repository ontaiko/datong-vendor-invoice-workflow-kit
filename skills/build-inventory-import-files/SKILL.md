...
- 本技能是唯一允許使用 Excel COM 的進貨流程階段，目的只限正式 `.xls` 範本套用、舊版 Excel 格式保存與成品驗證。
- 可在正式輸出時讀取已確認的輸入 `.xlsx`，但不得回寫或修改該來源檔。
- 不使用 Excel COM 產生或修改前段中間 `.xlsx`；中間檔必須由 `$extract-vendor-invoice-image`、`$match-product-catalog`、`$review-invoice-product-check` 以 Python/openpyxl 產生。
- 不使用 Excel COM 讀產品 CSV 或做相似比對；CSV 與比對已由前段技能完成。
