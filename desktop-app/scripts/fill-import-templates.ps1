...
$formatProbeWorkbook = $null
$formatProbeWorksheet = $null
$generalNumberFormatLocal = ""

try {
    $excel = New-Object -ComObject Excel.Application
    $excel.Visible = $false
    $excel.DisplayAlerts = $false
    $formatProbeWorkbook = $excel.Workbooks.Add()
    $formatProbeWorksheet = $formatProbeWorkbook.Worksheets.Item(1)
    $generalNumberFormatLocal = [string]$formatProbeWorksheet.Cells.Item(1, 1).NumberFormatLocal
    $formatProbeWorkbook.Close($false)
    Release-ComObjects @($formatProbeWorksheet, $formatProbeWorkbook)
    $formatProbeWorksheet = $null
    $formatProbeWorkbook = $null
    ...
    if ($isCompactPurchaseTemplate) {
        ...
        # Use Excel's locale-specific General format so integer prices display
        # as "315" instead of "315.", while decimal prices remain numeric.
        $purchaseWorksheet.Range("G2:G65536").NumberFormatLocal = $generalNumberFormatLocal
        $purchaseWorksheet.Columns.Item(7).ColumnWidth = 12
    }
    ...
    if (-not $isCompactPurchaseTemplate) {
        for ($index = 0; $index -lt $items.Count; $index++) {
            $row = $index + 2
            $expectedPrice = [double]$items[$index].cost
            $actualPrice = [double]$purchaseWorksheet.Cells.Item($row, 7).Value2
            $displayedPrice = [string]$purchaseWorksheet.Cells.Item($row, 7).Text
            if ([Math]::Abs($actualPrice - $expectedPrice) -gt 0.000001) {
                throw "Purchase unit price verification failed at row $row."
            }
            if ($displayedPrice.EndsWith(".") -or $displayedPrice.Contains("#")) {
                throw "Purchase unit price display verification failed at row ${row}: '$displayedPrice'."
            }
        }
    }
    ...
    if ($null -ne $formatProbeWorkbook) {
        $formatProbeWorkbook.Close($false)
    }
    if ($null -ne $excel) {
        $excel.Quit()
