...
def compare_item(
    ...
    for product in products:
        ...
        if score >= low_threshold and not has_meaningful_overlap(
            source_name, product["productName"], identity_tokens
        ):
            score *= 0.55
        candidate = {
            "productCode": product["productCode"],
        ...
        if score >= threshold:
            ...
            normal_scored.append(candidate)
        elif score >= low_threshold:
            candidate["tier"] = "low"
            low_scored.append(candidate)

    candidate_sort_key = lambda candidate: (-candidate["score"], candidate["productCode"])
    normal_scored.sort(key=candidate_sort_key)
    low_scored.sort(key=candidate_sort_key)
    normal_candidates = normal_scored[:max_candidates]
    low_candidates = low_scored[:max_low_candidates]
    result["similarCandidates"] = normal_candidates + low_candidates
    if normal_candidates:
        ...
        result["matchNote"] = "只找到低相似候選，僅供人工查看，不視為已建檔或合理相似商品。"
...
def main():
    ...
    parser.add_argument("--low-similar-threshold", type=float, default=0.55)
    parser.add_argument("--max-low-candidates", type=int, default=3)
    parser.add_argument(
        "--no-suggestion-txt",
        action="store_true",
    ...
    if not 0 <= args.low_similar_threshold < args.similar_threshold <= 1:
        parser.error("候選門檻必須符合 0 <= 低相似門檻 < 一般門檻 <= 1。")
    matched = [
        compare_item(
            item,
            products,
            ...
            args.max_candidates,
            args.low_similar_threshold,
            args.max_low_candidates,
        )
        for item in items
    ]
